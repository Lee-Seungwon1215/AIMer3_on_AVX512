...
/* PQClean's constant-time test annotation; a no-op when not under valgrind/ct. */
...
////////////////////////////////////////////////////////////////////////////////
static void crypto_sign_keypair_internal(uint8_t *pk, uint8_t *sk,
...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk)
{
  ...
  OQS_randombytes(pt, AIM2_NUM_BYTES_FIELD);
  OQS_randombytes(iv, AIM2_IV_SIZE);
  crypto_sign_keypair_internal(pk, sk, pt, iv);

  return 0;
}

static void crypto_sign_signature_internal(uint8_t *sig, size_t *siglen,
                                    const uint8_t *m, size_t mlen,
                                    const uint8_t *pre, size_t prelen,
                          ...
                          const uint8_t *ctx, size_t ctxlen, const uint8_t *sk)
{
  ...
  OQS_randombytes(randomness, SECURITY_BYTES);
#else
