...
int crypto_sign_signature(uint8_t *sig, size_t *siglen,
        ...
        const uint8_t *ctx_str, size_t ctx_len, const uint8_t *sk)
{
  if (ctx_len > 255)
  {
    return -1;
  ...
  uint8_t prefix[256];
  prefix[0] = (uint8_t)ctx_len;
  for (size_t pi = 0; pi < ctx_len; pi++)
  {
    prefix[1 + pi] = ctx_str[pi];
  }
  hash_instance ctx;
  ...
  run_phase_1(sign, commits,
              ...
              alpha_v_shares, sk, m, mlen, prefix, 1 + ctx_len);
...
int crypto_sign(uint8_t *sm, size_t *smlen,
        ...
        const uint8_t *ctx_str, size_t ctx_len, const uint8_t *sk)
{
  crypto_sign_signature(sm + mlen, smlen, m, mlen, ctx_str, ctx_len, sk);
...
int crypto_sign_verify(const uint8_t *sig, size_t siglen,
        const uint8_t *m, size_t mlen,
        const uint8_t *ctx_str, size_t ctx_len, const uint8_t *pk)
{
  if (ctx_len > 255)
  {
    return -1;
  ...
  hash_update(&ctx_h1, prefix, 1 + ctx_len);
        ...
        const uint8_t *ctx_str, size_t ctx_len, const uint8_t *pk)
{
  ...
  if (crypto_sign_verify(signature, CRYPTO_BYTES, message, message_len, ctx_str, ctx_len, pk))
  {
    return -1;
  }
