...
OQS_API void OQS_init(void) {
	...
	/* AIMer: route all SHA-3/SHAKE through the Intel AVX512VL Keccak (ported from
	 * liboqs 0.15.0) when the CPU supports AVX-512. Runs once before any hashing,
	 * so every AIMer variant (incl. _ref) gets the fast Keccak transparently. */
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX512)) {
		...
		extern void OQS_SHA3_set_callbacks(struct OQS_SHA3_callbacks *);
		extern void OQS_SHA3_x4_set_callbacks(struct OQS_SHA3_x4_callbacks *);
