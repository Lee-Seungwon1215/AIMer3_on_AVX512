...
// Paper-measurement AIMer v2 benchmark, CSV-compatible with AIMer v3.
...
static void usage(const char *program) {
	fprintf(stderr,
	        "usage: %s <ref|avx2|avx512> <AIMER-variant> <iters> "
	        "[warmup]\n",
	        program);
}
...
int main(int argc, char **argv) {
	...
	if (setenv("AIMER_IMPL", backend->name, 1) != 0 ||
	    setenv("AIMER_KECCAK", backend->name, 1) != 0) {
		perror("setenv");
		return 2;
	}
	OQS_init();
