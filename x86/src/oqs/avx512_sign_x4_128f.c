...
static void commit_and_expand_tape_x4(
    tape_t tapes[4], uint8_t commits[4][AIMER_COMMIT_SIZE],
    const uint8_t *salt, size_t rep, size_t first_party,
    const uint8_t *seeds[4])
{
  aimer_v3_avx512_shake128x4incctx ctx;
  const uint8_t prefix = HASH_PREFIX_5;
  const uint8_t rep_byte = (uint8_t)rep;
  const uint8_t party_bytes[4] = {
    (uint8_t)first_party,
    (uint8_t)(first_party + 1),
    (uint8_t)(first_party + 2),
    (uint8_t)(first_party + 3),
  };

  aimer_v3_avx512_shake128_x4_inc_init(&ctx);
  aimer_v3_avx512_shake128_x4_inc_absorb(
      &ctx, &prefix, &prefix, &prefix, &prefix, sizeof(prefix));
  aimer_v3_avx512_shake128_x4_inc_absorb(
      &ctx, salt, salt, salt, salt, AIMER_SALT_SIZE);
  aimer_v3_avx512_shake128_x4_inc_absorb(
      &ctx, &rep_byte, &rep_byte, &rep_byte, &rep_byte, sizeof(rep_byte));
  aimer_v3_avx512_shake128_x4_inc_absorb(
      &ctx, &party_bytes[0], &party_bytes[1],
      &party_bytes[2], &party_bytes[3], sizeof(party_bytes[0]));
  aimer_v3_avx512_shake128_x4_inc_absorb(
      &ctx, seeds[0], seeds[1], seeds[2], seeds[3], AIMER_SEED_SIZE);
  aimer_v3_avx512_shake128_x4_inc_finalize(&ctx);

  aimer_v3_avx512_shake128_x4_inc_squeeze(
      commits[0], commits[1], commits[2], commits[3],
      AIMER_COMMIT_SIZE, &ctx);
  aimer_v3_avx512_shake128_x4_inc_squeeze(
      (uint8_t *)&tapes[0], (uint8_t *)&tapes[1],
      (uint8_t *)&tapes[2], (uint8_t *)&tapes[3],
      sizeof(tape_t), &ctx);
  aimer_v3_avx512_shake128_x4_inc_ctx_release(&ctx);
}
                       ...
                       const uint8_t *pre, size_t prelen)
{
  ...
  for (size_t rep = 0; rep < AIMER_T; rep++)
  {
    ...
    tape_t delta, tapes[AIMER_N];
    memset(&delta, 0, sizeof(tape_t));

    for (size_t first_party = 0; first_party < AIMER_N; first_party += 4)
    {
      const uint8_t *seeds[4] = {
        nodes[rep][first_party + AIMER_N - 1],
        nodes[rep][first_party + AIMER_N],
        nodes[rep][first_party + AIMER_N + 1],
        nodes[rep][first_party + AIMER_N + 2],
      };
      commit_and_expand_tape_x4(
          &tapes[first_party], &commits[rep][first_party],
          sign->salt, rep, first_party, seeds);

      for (size_t lane = 0; lane < 4; lane++)
      {
        const size_t party = first_party + lane;

        gf_add(delta.pt_share, delta.pt_share, tapes[party].pt_share);
        for (size_t i = 0; i < AIMER_L; i++)
        {
          gf_add(delta.y_shares[i], delta.y_shares[i],
                 tapes[party].y_shares[i]);
        }
        for (size_t i = 0; i < AIMER_L + 1; i++)
        {
          gf_add(delta.a_shares[i], delta.a_shares[i],
                 tapes[party].a_shares[i]);
        }
        gf_add(delta.c_share, delta.c_share, tapes[party].c_share);

        if (party == AIMER_N - 1)
        {
          adjust_last_share(&sign->proofs[rep], &delta, &tapes[party], pt_gf,
                            sbox_outputs);
        }
      }
    }
    aim3_mpc_N(mult_chk[rep], lin, tapes, ct_gf);
                                       ...
                                       const uint8_t *pk)
{
  ...
  for (size_t rep = 0; rep < AIMER_T; rep++)
  {
    ...
    tape_t tapes[AIMER_N];
    mult_chk_t mult_checks[AIMER_N];
    memset(tapes, 0, sizeof(tapes));
    memset(mult_checks, 0, sizeof(mult_checks));

    static const uint8_t zero_seed[AIMER_SEED_SIZE] = {0};
    for (size_t first_party = 0; first_party < AIMER_N; first_party += 4)
    {
      const uint8_t *seeds[4];
      ...
      {
        const size_t party = first_party + lane;
        seeds[lane] = party == i_bar
                          ? zero_seed
                          : nodes[AIMER_N + party - 2];
      }

      commit_and_expand_tape_x4(
          &tapes[first_party], commits, sign->salt, rep, first_party, seeds);
      ...
      for (size_t lane = 0; lane < 4; lane++)
      {
        ...
        if (party == i_bar)
        {
          hash_update(&ctx_h1, sign->proofs[rep].missing_commitment,
                      AIMER_COMMIT_SIZE);
          recompute_missing_alpha(
              &sign->proofs[rep], alpha_shares[i_bar], alpha);
          continue;
        }

        hash_update(&ctx_h1, commits[lane], AIMER_COMMIT_SIZE);
        if (party == AIMER_N - 1)
        {
          recompute_last_share(&sign->proofs[rep], &tapes[party]);
        }
      }
    }

    aim3_mpc_N(mult_checks, lin, tapes, ct_gf);
    ...
    for (size_t party = 0; party < AIMER_N; party++)
    {
      ...
      const mult_chk_t *mult_chk = &mult_checks[party];
