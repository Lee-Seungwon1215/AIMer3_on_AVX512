...
// Four independent GF(2^128) elements are stored in the four 128-bit lanes of
// each ZMM register. No arithmetic crosses a party boundary.
...
void gf_mat_vec_mul_add_N(gf accum[AIMER_N], const gf in[AIMER_N],
                          const gf matrix[AIM3_NUM_BITS_FIELD]) {
	/* AIMER_N is 16 for AIM3-128f. Four ZMM values cover all parties. */
	__m512i value0 = _mm512_loadu_si512((const void *)in[0]);
	__m512i value1 = _mm512_loadu_si512((const void *)in[4]);
	__m512i value2 = _mm512_loadu_si512((const void *)in[8]);
	__m512i value3 = _mm512_loadu_si512((const void *)in[12]);
	__m512i out0 = _mm512_loadu_si512((const void *)accum[0]);
	__m512i out1 = _mm512_loadu_si512((const void *)accum[4]);
	__m512i out2 = _mm512_loadu_si512((const void *)accum[8]);
	__m512i out3 = _mm512_loadu_si512((const void *)accum[12]);
	...
	_mm512_storeu_si512((void *)accum[0], out0);
	_mm512_storeu_si512((void *)accum[4], out1);
	_mm512_storeu_si512((void *)accum[8], out2);
	_mm512_storeu_si512((void *)accum[12], out3);
}
